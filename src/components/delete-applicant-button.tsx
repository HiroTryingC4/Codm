...
export default function DeleteApplicantButton({
  ...
  async function handleRemove() {
    ...
    if (!res.ok) {
      ...
      setError(data.error || "Could not remove this applicant.");
      return;
    }
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-lg border border-red-300 dark:border-red-500/30 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors px-3 py-1.5 text-sm font-medium"
      >
        ...
        createPortal(
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            ...
            <div className="animate-fade-in-up relative w-full max-w-sm rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 shadow-2xl p-6 space-y-4">
              <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Remove Applicant</h2>
              <p className="text-sm text-neutral-600 dark:text-neutral-400">
                Remove <span className="font-medium text-neutral-900 dark:text-neutral-100">{applicantName}</span>?
                This will permanently delete their application and comments. This cannot be undone.
              </p>
              {error && (
      ...
      <SuccessModal
        ...
        message={`${applicantName} has been deleted.`}
      />
    </>
  );
}
